...
exports.contactMessageToSlack = functions.database.ref('/contact/{messageId}').onWrite(event => {
    const snapshot = event.data;
    const contact = snapshot.val();

    const message = `*${contact.name}*, ${contact.email} ${contact.phone}. *Message:* ${contact.message}`;
    return rp({
        method: 'POST',
